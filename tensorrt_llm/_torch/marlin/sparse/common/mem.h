...
__device__ inline void cp_async4_pred_zfill(
    void* smem_ptr, void const* glob_ptr, bool pred = true, bool const zfill = false)
{
    int const BYTES = 16;
...
__device__ inline void ldsm4_m(FragM& frag_m, void const* smem_ptr)
{
...
// Instruction for loading a full 16x16 matrix fragment of operand A from shared
// memory, directly in tensor core layout.
__device__ inline void ldsm4_t(FragA& frag_a, void const* smem_ptr)
{
    uint32_t* a = reinterpret_cast<uint32_t*>(&frag_a);
    uint32_t smem = static_cast<uint32_t>(__cvta_generic_to_shared(smem_ptr));
