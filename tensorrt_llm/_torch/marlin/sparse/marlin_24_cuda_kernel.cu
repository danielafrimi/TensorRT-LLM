...
template <int const num_bits,                         // weight bits
    ...
    int const stages,                                 // number of stages for the async global->shared
                                                      // fetch pipeline
    int const group_blocks = -1                       // number of consecutive 16x16 blocks
                                                      // with a separate quantization scale
    >
__global__ void Marlin_24(int4 const* __restrict__ A, // fp16 input matrix of shape mxk
    int4 const* __restrict__ B,                       // 4bit quantized weight matrix of shape kxn
    int4 const* __restrict__ meta,                    // 2bit metadata information about 2:4
                                                      // format on B
    int4* __restrict__ C,                             // fp16 output buffer of shape mxn
    int4 const* __restrict__ s,                       // fp16 quantization scales of shape
                                                      // (k/groupsize)xn
...
torch::Tensor gptq_marlin_24_gemm(torch::Tensor& a, torch::Tensor& b_q_weight, torch::Tensor& b_meta,
    torch::Tensor& b_scales, torch::Tensor& workspace, vllm::ScalarTypeId const b_q_type_id, int64_t size_m,
    int64_t size_n, int64_t size_k)
{
    TORCH_CHECK_NOT_IMPLEMENTED(false, "gptq_marlin_24_gemm(..) requires CUDA_ARCH >= 8.0");
...
#pragma unroll
    ...
    int4 const* meta_ptr[m_sh_iters];
#pragma unroll
...
#pragma unroll
            for (int i = 0; i < a_sh_wr_iters; i++)
            {
                cp_async4_pred(&sh_a_stage[a_sh_wr_trans[i]],
                    &A[a_gl_rd_delta_i * i + a_gl_rd + a_gl_rd_delta_o * a_off], a_sh_wr_pred[i]);
            }
...
#pragma unroll
                            for (int j1 = 0; j1 < 4; j1++)
                            {
                                reinterpret_cast<float*>(
                                    &frag_c)[4 * 2 * 4 * (i / 4) + 8 * j1 + 2 * j2 + 4 * ((i % 4) / 2) + i % 2]
                                    += __half2float(reinterpret_cast<__half*>(&c_red)[(j2 * 4 + j1)]);
                            }
                        }
                    }
                    if (!last)
...
#pragma unroll
                            for (int j1 = 0; j1 < 4; j1++)
                            {
                                reinterpret_cast<__half*>(&c)[(j2 * 4 + j1)] = __float2half(reinterpret_cast<float*>(
                                    &frag_c)[4 * 2 * 4 * (i / 4) + 8 * j1 + 2 * j2 + 4 * ((i % 4) / 2) + i % 2]);
                            }
                        }
    ...
    auto write_result = [&]()
    {
        ...
        int c_sh_rd
            = c_sh_stride_3 * (threadIdx.x / (2 * 2 * thread_n_blocks)) + (threadIdx.x % (2 * 2 * thread_n_blocks));

        int c_gl_wr_end = c_gl_stride * prob_m;

        auto write = [&](int idx, float c0, float c1, float c2, float c3, FragS& s0, float c4, float c5, float c6,
                         float c7, FragS& s1)
        {
            uint2 res[2];
...
#pragma unroll
            for (int i = 0; i < thread_m_blocks; i++)
            {
                ...
                write(wr + c_sh_stride, frag_c[i][0][0][1], frag_c[i][1][0][1], frag_c[i][2][0][1], frag_c[i][3][0][1],
                    frag_s[0][0], frag_c[i][0][0][3], frag_c[i][1][0][3], frag_c[i][2][0][3], frag_c[i][3][0][3],
                    frag_s[0][2]);
                write(wr + 4 * c_sh_stride_2, frag_c[i][0][1][0], frag_c[i][1][1][0], frag_c[i][2][1][0],
                    frag_c[i][3][1][0], frag_s[0][0], frag_c[i][0][1][2], frag_c[i][1][1][2], frag_c[i][2][1][2],
                    frag_c[i][3][1][2], frag_s[0][2]);
                write(wr + 4 * c_sh_stride_2 + c_sh_stride, frag_c[i][0][1][1], frag_c[i][1][1][1], frag_c[i][2][1][1],
                    frag_c[i][3][1][1], frag_s[0][0], frag_c[i][0][1][3], frag_c[i][1][1][3], frag_c[i][2][1][3],
                    frag_c[i][3][1][3], frag_s[0][2]);
...
#pragma unroll
                    for (int i = 0; i < thread_m_blocks; i++)
                    {
                        scale_floats(&frag_c[i][0][0][0], &frag_c[i][1][0][0], &frag_c[i][2][0][0], &frag_c[i][3][0][0],
                            frag_s[0][0], &frag_c[i][0][0][2], &frag_c[i][1][0][2], &frag_c[i][2][0][2],
                            &frag_c[i][3][0][2], frag_s[0][2]);

                        scale_floats(&frag_c[i][0][0][1], &frag_c[i][1][0][1], &frag_c[i][2][0][1], &frag_c[i][3][0][1],
                            frag_s[0][0], &frag_c[i][0][0][3], &frag_c[i][1][0][3], &frag_c[i][2][0][3],
                            &frag_c[i][3][0][3], frag_s[0][2]);

                        scale_floats(&frag_c[i][0][1][0], &frag_c[i][1][1][0], &frag_c[i][2][1][0], &frag_c[i][3][1][0],
                            frag_s[0][0], &frag_c[i][0][1][2], &frag_c[i][1][1][2], &frag_c[i][2][1][2],
                            &frag_c[i][3][1][2], frag_s[0][2]);

                        scale_floats(&frag_c[i][0][1][1], &frag_c[i][1][1][1], &frag_c[i][2][1][1], &frag_c[i][3][1][1],
                            frag_s[0][0], &frag_c[i][0][1][3], &frag_c[i][1][1][3], &frag_c[i][2][1][3],
                            &frag_c[i][3][1][3], frag_s[0][2]);
                    }
                }
            }
...
#define CALL_IF_2_4(NUM_BITS, THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, GROUP_BLOCKS)                         \
    else if (num_bits == NUM_BITS && thread_m_blocks == THREAD_M_BLOCKS && thread_n_blocks == THREAD_N_BLOCKS          \
        && thread_k_blocks == THREAD_K_BLOCKS && group_blocks == GROUP_BLOCKS)                                         \
    {                                                                                                                  \
        cudaFuncSetAttribute(                                                                                          \
            Marlin_24<NUM_BITS, THREADS, THREAD_N_BLOCKS, THREAD_M_BLOCKS, THREAD_K_BLOCKS, STAGES, GROUP_BLOCKS>,     \
...
void marlin_cuda_2_4(void const* A, void const* B, void const* meta, void* C, void* s, int prob_m, int prob_n,
    int prob_k, void* workspace, int num_bits, int groupsize = -1, int dev = 0, cudaStream_t stream = 0,
    int thread_k = -1, int thread_m = -1, int sms = -1, int max_par = 16)
{
    int tot_n = prob_n;
    ...
    TORCH_CHECK(prob_m % thread_m == 0, "prob_m = ", prob_m, " is not divisible by thread_m = ", thread_m);
    ...
    int4 const* meta_ptr = (int4 const*) meta;
    int4* C_ptr = (int4*) C;
    int4 const* s_ptr = (int4 const*) s;
    ...
    for (int i = 0; i < tot_n_blocks; i += max_m_blocks)
    {
        ...
        else
        {
            throw std::runtime_error("Unsupported shapes: MKN = [" + str(prob_m) + ", " + str(prob_k) + ", "
                + str(prob_n) + "]" + ", groupsize = " + str(groupsize) + ", thread_m_blocks = " + str(thread_m_blocks)
                + ", thread_n_blocks = " + str(thread_n_blocks) + ", thread_k_blocks = " + str(thread_k_blocks));
        }
    ...
    int64_t size_n, int64_t size_k)
{
    ...
    // Verify M
    TORCH_CHECK(size_m == a.size(0), "Shape mismatch: a.size(0) = " + str(a.size(0)) + ", size_m = " + str(size_m));

    // Verify K
    TORCH_CHECK(size_k == a.size(1), "Shape mismatch: a.size(1) = " + str(a.size(1)) + ", size_k = " + str(size_k));
    TORCH_CHECK(size_k % marlin_24::tile_size == 0,
        "size_k = " + str(size_k) + " is not divisible by tile_size = " + str(marlin_24::tile_size));
    TORCH_CHECK((size_k / marlin_24::tile_size / 2) == b_q_weight.size(0),
        "Shape mismatch: b_q_weight.size(0) = " + str(b_q_weight.size(0)) + ", size_k = " + str(size_k)
            + ", tile_size = " + str(marlin_24::tile_size));

    // Verify N
    TORCH_CHECK(
        b_scales.size(1) == size_n, "b_scales.size(1) = " + str(b_scales.size(1)) + ", size_n = " + str(size_n));
    TORCH_CHECK(b_q_weight.size(1) % marlin_24::tile_size == 0,
    ...
    TORCH_CHECK(size_n == actual_size_n, "size_n = " + str(size_n) + ", actual_size_n = " + str(actual_size_n));

    // Verify meta
    TORCH_CHECK(b_meta.size(0) == size_k / 8 / 2 / 2, "b_meta.size(0) = ", b_meta.size(0),
        " is not size_k / 8 / 2 / 2 = ", size_k / 8 / 2 / 2);
    TORCH_CHECK(b_meta.size(1) == size_n * 2, "b_meta.size(1) = ", b_meta.size(1), " is not size_n * 2 = ", size_n * 2);

    // Verify A device and strides
    TORCH_CHECK(a.device().is_cuda(), "A is not on GPU");
    TORCH_CHECK(a.is_contiguous(), "A is not contiguous");
    TORCH_CHECK(a.dtype() == torch::kFloat16, "A is not float16, currently only float16 is supported");
    ...
    TORCH_CHECK(b_scales.dtype() == torch::kFloat16, "A is not float16, currently only float16 is supported");
    ...
    if (b_scales.size(0) > 1)
    {
        TORCH_CHECK(size_k % b_scales.size(0) == 0,
            "size_k = " + str(size_k) + ", is not divisible by b_scales.size(0) = " + str(b_scales.size(0)));
        groupsize = size_k / b_scales.size(0);
        groupsize /= 2; // Because of 24
    }

    // Verify groupsize
    TORCH_CHECK(groupsize == -1 || groupsize == 64, "Unexpected groupsize = " + str(groupsize));

    // Verify workspace size
    TORCH_CHECK(size_n % marlin_24::min_thread_n == 0,
        "size_n = " + str(size_n) + ", is not divisible by min_thread_n = " + str(marlin_24::min_thread_n));
    int min_workspace_size = (size_n / marlin_24::min_thread_n) * marlin_24::max_par;
    TORCH_CHECK(workspace.numel() >= min_workspace_size,
        "workspace.numel = " + str(workspace.numel()) + " is below min_workspace_size = " + str(min_workspace_size));

    int dev = a.get_device();
    marlin_24::marlin_cuda_2_4(a.data_ptr(), b_q_weight.data_ptr(), b_meta.data_ptr(), c.data_ptr(),
        b_scales.data_ptr(), size_n, size_m, size_k, workspace.data_ptr(), b_q_type.size_bits(), groupsize, dev,
