...
int const SHARED_MEM = 96 * 1024; // max shared memory on compute capability 8.6 (< 8.0)
...
#define __CALL_IF(THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, GROUP_BLOCKS, NUM_THREADS)                        \
    ...
    {                                                                                                                  \
        ...
        Marlin<NUM_THREADS, THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, STAGES, GROUP_BLOCKS>                   \
            <<<blocks, NUM_THREADS, SHARED_MEM, stream>>>(A_ptr, B_ptr, C_ptr, s_ptr, prob_m, prob_n, prob_k, locks);  \
    }
...
void marlin_cuda(void const* A, void const* B, void* C, void* s, int prob_m, int prob_n, int prob_k, void* workspace,
    int groupsize = -1, int dev = 0, cudaStream_t stream = 0, int thread_k = -1, int thread_n = -1, int sms = -1,
    int max_par = 16)
{
    int tot_m = prob_m;
    ...
    torch::Tensor& workspace, int64_t size_m, int64_t size_n, int64_t size_k)
{
    ...
    TORCH_CHECK(size_k % marlin_dense::tile_size == 0,
        "size_k = " + str(size_k) + " is not divisible by tile_size = " + str(marlin_dense::tile_size));
    ...
    TORCH_CHECK(b_q_weight.size(1) % marlin_dense::tile_size == 0,
        "b_q_weight.size(1) = " + str(b_q_weight.size(1))
            + " is not divisible by tile_size = " + str(marlin_dense::tile_size));

    int actual_size_n = (b_q_weight.size(1) / marlin_dense::tile_size) * marlin_dense::pack_factor_4bit;
    ...
    TORCH_CHECK(size_n % marlin_dense::min_thread_n == 0,
        "size_n = " + str(size_n) + ", is not divisible by min_thread_n = " + str(marlin_dense::min_thread_n));
    int min_workspace_size = (size_n / marlin_dense::min_thread_n) * marlin_dense::max_par;
    TORCH_CHECK(workspace.numel() >= min_workspace_size,
        "workspace.numel = " + str(workspace.numel()) + " is below min_workspace_size = " + str(min_workspace_size));

    int dev = a.get_device();
    marlin_dense::marlin_cuda(a.data_ptr(), b_q_weight.data_ptr(), c.data_ptr(), b_scales.data_ptr(), size_m, size_n,
        size_k, workspace.data_ptr(), groupsize, dev, at::cuda::getCurrentCUDAStream(dev), thread_k, thread_n, sms,
        marlin_dense::max_par);
