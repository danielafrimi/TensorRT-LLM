...
__device__ inline FragB dequant_per_group(int q, FragS_GROUP& frag_s, int i)
{
    ...
    *reinterpret_cast<half2*>(&t1) = __hfma2(
        *reinterpret_cast<half2*>(&t1), *reinterpret_cast<half2 const*>(&MUL), *reinterpret_cast<half2 const*>(&ADD));
    ...
    *reinterpret_cast<half2*>(&t0) = __hfma2(*reinterpret_cast<half2*>(&t0), *reinterpret_cast<half2*>(&double_s),
        *reinterpret_cast<half2 const*>(&MAGIC_NUM));
    *reinterpret_cast<half2*>(&t1) = __hfma2(*reinterpret_cast<half2*>(&t1), *reinterpret_cast<half2*>(&double_s),
        *reinterpret_cast<half2 const*>(&MAGIC_NUM));
...
template <int const threads,                       // number of threads in a threadblock
    ...
    >
__global__ void Marlin(int4 const* __restrict__ A, // int8 input matrix of shape mxk
    ...
    float const* __restrict__ s_tok,               // fp32 activation per-token quantization
                                                   // scales of shape mx1
    int4 const* __restrict__ s_ch,                 // fp32 weight per-channel quantization
                                                   // scales of shape 1xn
    int4 const* __restrict__ s_group,              // fp16 weight per-group quantization
...
)
{
    ...
    int s_ch_sh_rd = 16 * ((threadIdx.x / 32) % (thread_n_blocks / 4)) + 2 * ((threadIdx.x % 32) % 4);
    ...
    if constexpr (group_blocks != -1)
    {
        s_group_gl_rd = s_group_gl_stride * ((thread_k_blocks * slice_row) / group_blocks)
            + s_group_sh_stride * slice_col + threadIdx.x;
        s_group_sh_wr = threadIdx.x;
        // NOTE(HandH1998): s_group_sh_rd is related to mma output C
        s_group_sh_rd = 8 * ((threadIdx.x / 32) % (thread_n_blocks / 4)) + (threadIdx.x % 32) / 4;
...
#pragma unroll
            ...
            if constexpr (group_blocks != -1)
            {
                if (pipe % (group_blocks / thread_k_blocks) == 0)
                {
                    ...
                    if (s_group_sh_wr_pred)
                        cp_async4(&sh_s_group_stage[s_group_sh_wr], &s_group[s_group_gl_rd]);
    ...
    auto fetch_to_registers = [&](int k, int pipe)
    {
        ...
        if constexpr (group_blocks != -1)
        {
            int4* sh_s_group_stage = sh_s_group
                + s_group_sh_stage * ((group_blocks / thread_k_blocks) * (pipe / (group_blocks / thread_k_blocks)));
            reinterpret_cast<int4*>(&frag_s_group[k % 2])[0] = sh_s_group_stage[s_group_sh_rd];
        }
        int4* sh_a_stage = sh_a + a_sh_stage * pipe;
#pragma unroll
...
#pragma unroll
                ...
                {
                    int wr = d_sh_wr + 8 * j;
                    write(wr + (4 * d_sh_stride) * 0 + 0, frag_c[i][j][0][0], frag_c[i][j][0][1], frag_s_tok[i][0],
                        frag_s_ch[j / 2][2 * (j % 2) + 0]);
                    write(wr + (4 * d_sh_stride) * 8 + 0, frag_c[i][j][0][2], frag_c[i][j][0][3], frag_s_tok[i][1],
                        frag_s_ch[j / 2][2 * (j % 2) + 0]);
                    write(wr + (4 * d_sh_stride) * 0 + 4, frag_c[i][j][1][0], frag_c[i][j][1][1], frag_s_tok[i][0],
                        frag_s_ch[j / 2][2 * (j % 2) + 1]);
                    write(wr + (4 * d_sh_stride) * 8 + 4, frag_c[i][j][1][2], frag_c[i][j][1][3], frag_s_tok[i][1],
                        frag_s_ch[j / 2][2 * (j % 2) + 1]);
                }
...
#pragma unroll
                    for (int i = 0; i < thread_m_blocks; i++)
                    {
                        frag_s_tok[i][0] = *reinterpret_cast<float*>(&sh_s_tok[16 * i + 2 * s_tok_sh_rd]);
                        frag_s_tok[i][1] = *reinterpret_cast<float*>(&sh_s_tok[16 * i + 2 * s_tok_sh_rd + 1]);
                    }
...
int const USER_THREADS = 256; // Note: This is only used with user-provided thread_k/n
int const STAGES = 4;         // 4 pipeline stages fit into shared memory
    ...
    {                                                                                                                  \
        ...
        Marlin<NUM_THREADS, THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, STAGES, GROUP_BLOCKS>                   \
            <<<blocks, NUM_THREADS, max_shared_mem, stream>>>(                                                         \
                A_ptr, B_ptr, C_ptr, D_ptr, s_tok_ptr, s_ch_ptr, s_group_ptr, prob_m, prob_n, prob_k, locks);          \
    }
...
void marlin_qqq_cuda(void const* A, void const* B, void* C, void* D, void* s_tok, void* s_ch, void* s_group, int prob_m,
    int prob_n, int prob_k, void* workspace, int groupsize = -1, int dev = 0, cudaStream_t stream = 0,
    int thread_k = -1, int thread_n = -1, int sms = -1, int max_par = 16)
{
    ...
    if (!is_valid_config(th_config, prob_m, prob_n, prob_k))
    {
        throw std::runtime_error("Invalid thread config: thread_k = " + str(th_config.thread_k)
            + ", thread_n = " + str(th_config.thread_n) + ", num_threads = " + str(th_config.num_threads)
    ...
    int4* C_ptr = (int4*) C;
    int4* D_ptr = (int4*) D;
    float const* s_tok_ptr = (float const*) s_tok;
    int4 const* s_ch_ptr = (int4 const*) s_ch;
    int4 const* s_group_ptr = (int4 const*) s_group;
...
torch::Tensor marlin_qqq_gemm(torch::Tensor const& a, torch::Tensor const& b_q_weight, torch::Tensor const& s_tok,
    torch::Tensor const& s_ch, torch::Tensor const& s_group, torch::Tensor& workspace, int64_t size_m, int64_t size_n,
    int64_t size_k)
{
    // Verify M
    TORCH_CHECK(size_m == a.size(0), "Shape mismatch: a.size(0) = " + str(a.size(0)) + ", size_m = " + str(size_m));
    TORCH_CHECK(
        size_m == s_tok.numel(), "Shape mismatch: s_tok.numel() = " + str(s_tok.numel()) + ", size_m = " + str(size_m));

    // Verify K
    TORCH_CHECK(size_k == a.size(1), "Shape mismatch: a.size(1) = " + str(a.size(1)) + ", size_k = " + str(size_k));
    TORCH_CHECK(
        size_k % tile_size == 0, "size_k = " + str(size_k) + " is not divisible by tile_size = " + str(tile_size));
    ...
    // Verify groupsize
    TORCH_CHECK(groupsize == -1 || groupsize == 128, "Unexpected groupsize = " + str(groupsize));

    // Verify N
    TORCH_CHECK(
        s_ch.numel() == size_n, "Shape mismatch: s_ch.numel() = " + str(s_ch.numel()) + ", size_n = " + str(size_n));
    TORCH_CHECK(b_q_weight.size(1) % tile_size == 0,
        "b_q_weight.size(1) = " + str(b_q_weight.size(1)) + " is not divisible by tile_size = " + str(tile_size));
    if (groupsize != -1)
    {
        TORCH_CHECK(s_group.size(1) == size_n,
            "Shape mismatch: s_group.size(1) = " + str(s_group.size(1)) + ", size_n = " + str(size_n));
        TORCH_CHECK(size_k % s_group.size(0) == 0,
            "size_k = " + str(size_k) + ", is not divisible by s_group.size(0) = " + str(s_group.size(0)));
    }

    int actual_size_n = (b_q_weight.size(1) / tile_size) * pack_factor_4bit;
    TORCH_CHECK(
        size_n == actual_size_n, "Shape mismatch: size_n = " + str(size_n) + ", actual_size_n = " + str(actual_size_n));
    ...
    TORCH_CHECK(s_group.dtype() == torch::kFloat16, "s_group's dtype is not float16");

    // Verify workspace size
    TORCH_CHECK(size_n % min_thread_n == 0,
        "size_n = " + str(size_n) + ", is not divisible by min_thread_n = " + str(min_thread_n));
    ...
    marlin_qqq_cuda(a.data_ptr(), b_q_weight.data_ptr(), c.data_ptr(), d.data_ptr(), s_tok.data_ptr(), s_ch.data_ptr(),
        s_group.data_ptr(), size_m, size_n, size_k, workspace.data_ptr(), groupsize, dev,
